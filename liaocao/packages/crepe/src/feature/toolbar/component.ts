...
import { boldIcon, codeIcon, italicIcon, linkIcon, strikethroughIcon, robot1Icon } from '../../icons'
  ...
  return html`<host>
    ...
    <button
      ...
      onmousedown=${onClick((ctx) => {
        ...
        ctx.get(assistantAPI.key).addLink(selection.from, selection.to)
        hide?.()
      })}
    >
      ${config?.linkIcon?.() ?? robot1Icon}
    </button>
  </host>`
}
